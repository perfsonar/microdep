...
pstracetree/unibuild-repo/RPMS: pstracetree unibuild-compose.yml
	...
	cd pstracetree && docker compose -f unibuild-compose.yml run el9 bash -c "dnf update && unibuild build"

pstracetree/unibuild-repo/Packages: pstracetree unibuild-compose.yml
	@echo "Build pstracetree U22 deb packages..."
	cp unibuild-compose.yml pstracetree/
	cd pstracetree && docker compose -f unibuild-compose.yml run u22_amd64 bash -c "apt update && unibuild build"

unibuild-repo/RPMS: unibuild-compose.yml
	@echo "Build Microdep EL9 rpms..."
	docker compose -f unibuild-compose.yml run el9 bash -c "dnf update && unibuild build"

unibuild-repo/Packages: unibuild-compose.yml
	@echo "Build Microdep U22 deb packages..."
	docker compose -f unibuild-compose.yml run u22_amd64 bash -c "apt update && unibuild build"
...
rpm-test:  clean-rpm-test rpm-build
...
deb-test:  clean-deb-test deb-build
